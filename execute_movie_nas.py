...
def run_progress(command, label, input=None, timeout=1800):
    """Report elapsed time while an external verifier runs; preserve its timeout."""
        ...
        try:
            while True:
                ...
                try:
                    ...
                    input = None  # communicate retains buffered input across waits.
...
class Radarr:
    ...
    def verify_file(self, path, expected_hash):
        self.visible(path)
        result = run_progress(
            ['docker', 'exec', 'radarr', 'sha256sum', '--', path], 'Radarr file-content verification')
...
def execute(base, execution_id, live, radarr, log, transport=None, recovery=None):
    ...
    progress('Verifying source contents on Latitude')
...
for p in (src, dst):
    require(len(p.parts) == 6 and p.parts[:4] == ('/', 'volume3', 'media04', 'Movies')
